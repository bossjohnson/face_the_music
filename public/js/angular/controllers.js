...
app.controller('sequenceRowCtrl', sequenceRowCtrl);
...
function faceCtrl($scope, $timeout, $http, $rootScope, faceService) {
    $http.get('/faces/' + $scope.faceId).then(function(data) {
        ...
        $scope.play = function() {
            ...
            // Right Eye Width
            osc = faceService.makeTone((rightEye.outerX - rightEye.innerX) / 2 + (nose.area / 125), chain);
            $scope.oscillators.push(osc);

            // Right Eye Height
            osc = faceService.makeTone((rightEye.bottomY - rightEye.topY) / 2 - (nose.area / 125), chain);
            $scope.oscillators.push(osc);
...
function sequenceRowCtrl($scope) {
    $scope.faces = [];
